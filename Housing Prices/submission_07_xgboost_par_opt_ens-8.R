...
search_grid <- expand.grid(subsample = c(0.75, 1), 
                           colsample_bytree = c(0.8, 1),
                           eta = c(0.1, 0.2, 0.4),
                           nrounds = c(200, 300, 400, 500, 800),
                           max.depth = c(2, 4, 8, 12))

result <- search_grid

# Run Search Loop (no seed)
rmse <- c()
for (i in c(1:nrow(search_grid))) {
      
      xgboostModelCV <- xgb.cv(data =  dtrain, 
                               nrounds = search_grid$nrounds[i], 
                               nfold = 4, 
                               showsd = TRUE, 
                               metrics = "rmse", 
                               verbose = FALSE, 
                               ...
                               "max.depth" = search_grid$max.depth[i], 
      ...
      rmse <- append(rmse,  tail(xvalidationScores$test.rmse.mean, 1))
      print(paste0(i, ": ", round(rmse[i], 0)))
}
result <- cbind(result, rmse)

ggplot(result, aes(x = rmse)) + geom_histogram()
ggplot(result, aes(y = rmse, x = as.factor(eta))) + geom_boxplot()
ggplot(result, aes(y = rmse, x = as.factor(max.depth))) + geom_boxplot()
ggplot(result, aes(y = rmse, x = as.factor(nrounds))) + geom_boxplot()
ggplot(result, aes(y = rmse, x = as.factor(subsample))) + geom_boxplot()
ggplot(result, aes(y = rmse, x = as.factor(colsample_bytree))) + geom_boxplot()

min(result$rmse)

# Best results parameters - lowest 8 models
final_grid <- filter(result, rmse < 28300)

# Test File
dtest <- xgb.DMatrix(as.matrix(tt))

# Train best few models, predict and later ensemble
for (i in c(1:nrow(final_grid))) {
      
      # Create model
      fit_gbtree <- xgb.train(data = dtrain, 
                              max.depth = final_grid$max.depth[i],
                              eta = final_grid$eta[i], 
                              nthread = 2,
                              watchlist = watchlist, 
                              nround = final_grid$nrounds[i], 
                              verbose = TRUE, 
                              subsample = final_grid$subsample[i],
                              colsample_bytree = final_grid$colsample_bytree[i])
      
      # Predict and add up (later: average predictions)
      if (i == 1) {
            ptt <- predict(fit_gbtree, newdata = dtest)
      }
      else {
            ptt <- ptt + predict(fit_gbtree, newdata = dtest)
      }

}

# Average predictions
ptt <- ptt / nrow(final_grid)
...
write_csv(df_tt, "submission_07.csv")

# Submission_07: Score: 0.14323 (prev. 0.14390, improved by 18 places)

# There are diminishing returns on that approach! Next step: feature engineering!
